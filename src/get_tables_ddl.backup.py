...
def _render_struct_type(fields: List[SchemaField], level: int) -> str:
    ...
    for idx, f in enumerate(fields):
        ...
        field_type_rendered = _render_type_for_field(
            f, level + 1)  # may be multiline
...
def _render_table_options(tbl: bigquery.Table) -> Optional[str]:
    opts = []
    if tbl.description:
        opts.append(f'description="{tbl.description.replace("\"", r"\\\"")}"')
...
def generate_table_ddl(client: bigquery.Client, project_id: str, dataset_id: str, table_id: str) -> str:
...
def get_table_ddl(project_id: str, dataset_id: str, table_id: str, color_mode: str = "auto", out_path: Optional[str] = None) -> None:
    client = bigquery.Client(project=project_id)
    try:
        ddl = generate_table_ddl(client, project_id, dataset_id, table_id)
        pretty_sql = print_pretty_colored_ddl(ddl, color_mode=color_mode)

        if out_path:
            # Always write clean, uncolored pretty SQL to file
            with open(out_path, "w", encoding="utf-8") as f:
                f.write(pretty_sql + "\n")
            # Inform user to where it was written (no ANSI in this line)
            print(f"\nWrote pretty DDL to: {out_path}")
    except Exception as e:
        print(f"Error retrieving DDL for {table_id}: {e}")
        raise
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate pretty, nested-formatted, colorized DDL for a BigQuery table (including constraints)."
    )
    ...
    parser.add_argument(
        "--out", metavar="PATH",
        help="Write pretty (uncolored) SQL to file. Example: --out table.ddl.sql"
    )
    args = parser.parse_args()

    missing = [k for k, v in vars(args).items() if v is None]
    if missing:
        raise SystemExit(f"Error: Missing required arguments: {', '.join(missing)}")
