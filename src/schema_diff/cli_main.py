...
def main() -> None:
    ...
    if len(sys.argv) > 1 and sys.argv[1] in ddl_commands:
        # Handle DDL, config, and generate subcommands
        parser = argparse.ArgumentParser(
            formatter_class=ColorDefaultsFormatter,
            description="Schema-diff tools: DDL generation, configuration, and schema generation",
        )
        ...
        # Add schema generation subcommands
        from .cli_generate import add_generate_subcommands

        add_generate_subcommands(subparsers)
