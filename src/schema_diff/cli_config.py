...
class SchemadiffConfig:
    ...
    exclude_patterns: list[str] = field(
        default_factory=lambda: [".*_temp", ".*_staging", "test_.*"]
    )
    ...
    @classmethod
    def load(cls, config_path: str | None = None) -> SchemadiffConfig:
    ...
    def _load_from_file(self, config_path: str) -> None:
        ...
        try:
            ...
            print(
                "Warning: PyYAML not installed, skipping config file", file=sys.stderr
            )
            return
    ...
    def save(self, config_path: str) -> None:
        ...
        try:
            ...
            raise ImportError("PyYAML required to save configuration") from None
...
def add_config_subcommands(subparsers) -> None:
    ...
    config_show.add_argument(
        "--config", metavar="PATH", help="Path to config file (default: auto-discover)"
    )
    ...
    config_init.add_argument(
        "config_path",
        nargs="?",
        default="schema-diff.yml",
        help="Path for new config file (default: schema-diff.yml)",
    )
    config_init.add_argument("--project", help="Default BigQuery project")
    config_init.add_argument("--dataset", help="Default BigQuery dataset")
    config_init.add_argument(
...
def cmd_config_init(args) -> None:
    ...
    try:
        ...
        if config_path.exists() and not args.force:
            print(
                f"Error: Config file {config_path} already exists. Use --force to overwrite.",
                file=sys.stderr,
            )
            sys.exit(1)
